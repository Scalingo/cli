...
`,
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			if c.NumFlags() == 0 {
				...
				if autoDeploy && noAutoDeploy {
					errorQuitWithHelpMessage(errors.New("cannot define both auto-deploy and no-auto-deploy"), ctx, c, "integration-link-create")
				}
				...
				if deployReviewApps && noDeployReviewApps {
					errorQuitWithHelpMessage(errors.New("cannot define both deploy-review-apps and no-deploy-review-apps"), ctx, c, "integration-link-create")
				}

				destroyOnClose := c.Bool("destroy-on-close")
				noDestroyOnClose := c.Bool("no-destroy-on-close")
				if destroyOnClose && noDestroyOnClose {
					errorQuitWithHelpMessage(errors.New("cannot define both destroy-on-close and no-destroy-on-close"), ctx, c, "integration-link-create")
				}
				...
				if destroyOnStale && noDestroyOnStale {
					errorQuitWithHelpMessage(errors.New("cannot define both destroy-on-stale and no-destroy-on-stale"), ctx, c, "integration-link-create")
				}
				...
				if allowReviewAppsFromForks && noAllowReviewAppsFromForks {
					errorQuitWithHelpMessage(errors.New("cannot define both allow-review-apps-from-forks and no-allow-review-apps-from-forks"), ctx, c, "integration-link-create")
				}

				awareOfSecurityRisks := c.Bool("aware-of-security-risks")

				if awareOfSecurityRisks && !c.IsSet("allow-review-apps-from-forks") {
					errorQuitWithHelpMessage(errors.New("flag --aware-of-security-risks must be used in conjunction with --allow-review-apps-from-forks"), ctx, c, "integration-link-create")
				}
	...
	integrationLinkUpdateCommand = cli.Command{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			noAutoDeploy := c.Bool("no-auto-deploy")
			if autoDeploy && noAutoDeploy {
				errorQuitWithHelpMessage(errors.New("cannot define both auto-deploy and no-auto-deploy"), ctx, c, "integration-link-update")
			}
			deployReviewApps := c.Bool("deploy-review-apps")
			noDeployReviewApps := c.Bool("no-deploy-review-apps")
			if deployReviewApps && noDeployReviewApps {
				errorQuitWithHelpMessage(errors.New("cannot define both deploy-review-apps and no-deploy-review-apps"), ctx, c, "integration-link-update")
			}
			destroyOnClose := c.Bool("destroy-on-close")
			noDestroyOnClose := c.Bool("no-destroy-on-close")
			if destroyOnClose && noDestroyOnClose {
				errorQuitWithHelpMessage(errors.New("cannot define both destroy-on-close and no-destroy-on-close"), ctx, c, "integration-link-update")
			}
			destroyOnStale := c.Bool("destroy-on-stale")
			noDestroyOnStale := c.Bool("no-destroy-on-stale")
			if destroyOnStale && noDestroyOnStale {
				errorQuitWithHelpMessage(errors.New("cannot define both destroy-on-stale and no-destroy-on-stale"), ctx, c, "integration-link-update")
			}
			...
			if allowReviewAppsFromForks && noAllowReviewAppsFromForks {
				errorQuitWithHelpMessage(errors.New("cannot define both allow-review-apps-from-forks and no-allow-review-apps-from-forks"), ctx, c, "integration-link-update")
			}

			awareOfSecurityRisks := c.Bool("aware-of-security-risks")

			if awareOfSecurityRisks && !c.IsSet("allow-review-apps-from-forks") {
				errorQuitWithHelpMessage(errors.New("flag --aware-of-security-risks must be used in conjunction with --allow-review-apps-from-forks"), ctx, c, "integration-link-update")
			}
