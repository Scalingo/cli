...
func ReadPrivateKeysFromAgent(ctx context.Context) ([]ssh.Signer, io.Closer, error) {
	agentPath := os.Getenv("SSH_AUTH_SOCK")
	agentHandler, err := net.Dial("unix", agentPath)
	if err != nil {
		return nil, nil, errors.Newf(ctx, "Fail to communicate with SSH agent: %v\nPlease precise the SSH key you want to use with the flag -i", err)
	}
	...
	if err != nil {
		return nil, nil, errors.Newf(ctx, "Fail to access SSH keys through SSH Agent: %v\n Please precise the SSH key you want to use with the flag -i", err)
	}
